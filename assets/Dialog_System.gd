...
##### SETUP #####
## Paths ##
var dialogues_folder = 'res://dialog' # Folder where the JSON files will be stored
...
var choice_height : int = 10 # Choice label's height
var choice_width : int = 60 # Choice label's width
var choice_margin_vertical : int = 2 # Vertical space (in pixels) between the bottom border of the dialogue frame and the last question (affectd by the 'label_margin')
var choice_margin_horizontal : int = 2 # Horizontal space (in pixels) between the border (set in 'choice_node_alignment') of the dialogue frame and the questions (affectd by the 'label_margin')
...
var sprite_offset : Vector2 = Vector2(30, 22) # Used for polishing avatars' position. Can use negative values.
...
var characters_folder = 'res://images/characters'
...
func load_image(char_sprite, image):
	char_sprite.texture = load('%s/%s' % [characters_folder, image])
...
func question(text, options, next):
	...
	for a in options:
		...
		choices.get_child(n).rect_position.x = (70) * n
		if wait_time > 0:
